...
tab=Table(names=['coRactr_'+base,'coDectr_'+base,'coCtrint_'+base,'coDvhel'
...
tab['coDvhel'].unit = 'km / s'
tab['coDvhel'].description = 'Add this to go from LSR to Barycentric frame'
...
#         if os.path.isfile(gal+'.'+line+'.'+altbase+'_mk12_dil.flux.out'):
#             dat = np.genfromtxt(gal+'.'+line+'.'+altbase+'_mk12_dil.flux.out',max_rows=1,
#                 comments=';',usecols=[1,4])
#             if dat[0] != 0.0 and dat[1] != 0.0:
#                 tab[lbl+'Mask12_di_natv'][j] = dat[0] * convfac
#                 tab[lbl+'eMask12_di_natv'][j] = dat[1] * convfac
#         if os.path.isfile(gal+'.'+line+'.'+altbase+'_mk12_2d.flux.out'):
#             dat = np.genfromtxt(gal+'.'+line+'.'+altbase+'_mk12_2d.flux.out',max_rows=1,
#                 comments=';',usecols=[1,4])
#             if dat[0] != 0.0 and dat[1] != 0.0:
#                 tab[lbl+'Mask12_2d_natv'][j] = dat[0] * convfac
#                 tab[lbl+'eMask12_2d_natv'][j] = dat[1] * convfac
